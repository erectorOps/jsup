...
SortableTable.prototype.initHeader = function (oSortTypes) {
	...
	for (var row = 0; row < this.tHead.rows.length; row++) {
		...
		for (var column = 0; column < cells.length; column++) {
			...
			if (!c.classList.contains("sortabletable-header")) c.classList.add("sortabletable-header");
...
SortableTable.prototype.updateHeaderArrows = function () {
	if (! this.tHead) { return; }
	for (let row = 0; row < this.tHead.rows.length; row++) {
		let cells = this.tHead.rows[row].cells;
		for (let column = 0; column < cells.length; column++) {
			let c = cells[column];
			if (this.getSortType(row, column) == 'None') { continue; }
			c.classList.remove("sortabletable-desc");
			c.classList.remove("sortabletable-asc");
			if (row == this.sortRow && column == this.sortColumn) {
			  c.classList.add("sortabletable-" + (this.descending ? "desc" : "asc"));
			}
		}
	}
};
...
SortableTable.toDate = function (s) {
	var s2 = s.split("-").join("/");
	return new Date(Date.parse(s2));
};

SortableTable.toNumber = function (s) {
  return parseFloat(s.split(',').join(''));
};
